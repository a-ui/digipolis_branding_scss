...
gulp.task('cdn', function (callback) {
    ...
    fs.rename('dist/aws', 'dist/' + version, function (err) {
        if (err) {
            ...
            console.log('\n----++++ https://cdn.antwerpen.be/digipolis_branding_scss/' + version + '/main.min.css ++++----\n');
            callback();
        }
    });
});
...
gulp.task('readme', function() {
    ...
    return gulp.src(['README.md'])
        .pipe(replace(/digipolis_branding_scss\/[0-9]\.[0-9]\.[0-9]\/main.min.css/g, 'digipolis_branding_scss/' + version + '/main.min.css'))
        .pipe(gulp.dest('./'));
});
